...
- 可移植性交接参考：`docs/PORTABILITY_ROADMAP.md`
...
`CodeIsland` 则是下一阶段 Linux 壳层重构最值得借鉴的公开参考，重点包括：

- 展开态按 provider 分组
- 紧凑的 section header 与数量标识
- provider 专属像素角色
- 更密集但更清晰的任务 / 摘要 / 操作层级
...
- Codex 桥接层现在会严格尊重 `approval_policy = "never"`，正常编码流程不应该再凭空弹出需要你手动同意的审批卡
...
- 会话卡正在从“单句摘要”过渡到紧凑的三行最近对话模型，并显式标注 `You` / provider
...
因此，下一执行切片应继续推进基于工件的恢复，并进一步把 UI 过渡到按 provider 分组的渲染。

这意味着：

- 展开态会话默认按 provider 分组
- 每个 provider section header 自己承担额度与运行数展示
- section 折叠状态会保存在 shell prefs 中
- 折叠态会在所有 provider 中选出“当前最重要的一条会话”来高亮
...
当前归一化后的 provider 集合是：

- `claude`
- `codex`
- `gemini`

内部 helper 的组织也正在向更易移植的形态收敛，目标是让未来这些平台尽可能复用同一套模型：
...
- 二进制发现
- transcript / 本地工件恢复
- usage 和 recent-dialogue 的归一化抽取
